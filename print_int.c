...
{
    spe->printed += ft_putchar('-');
    spe->printed += ft_fill('0', k);
    print_precision(str + 1, 10, spe);
}
...
void print_int(ft_specs *spe, va_list *argptr)
{
    ...
    str = ft_itoa(n);
    if (spe->width > (int)ft_strlen(str))
        k = spe->width - ft_strlen(str);
    if (spe->prec > (int)ft_strlen(str))
        spe->fillzero = spe->prec - ft_strlen(str);
    set_spec(ft_strlen(str), str, spe);
